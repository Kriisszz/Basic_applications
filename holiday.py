...
while True:
    try:
        ...
        if city_flight.lower()=="abudhabi":
            ...
            break
        else:city_flight=str(input("Destination can only be Abudhabi,Phuket,Amsterdam or Brussels."))
    except ValueError:
        print("Oops!  That was no valid number.  Try again...")  
while True:
    try:
        num_nights=int(input("Please add the number of nights you are going to spend there."))
        if num_nights>0:   
            break
        else:print("Please add a positive number!")
    except ValueError:
        print("Oops!  That was no valid number.  Try again...") 
while True:
    try:
        rental_days=int(input("Please add the number of days you are going to rent a car for."))
        if rental_days>0:   
            break
        else:print("Please add a positive number!")
    except ValueError:
...
if city_flight.lower()=="abudhabi":
        flight_cost=abudhabi_hotelprice
        rental_cost=abudhabi_rentalprice
elif city_flight.lower()=="phuket":
        flight_cost=phuket_hotelprice
        rental_cost=phuket_rentalprice
elif city_flight.lower()=="brussels":
        flight_cost=brussels_hotelprice
        rental_cost=brussels_rentalprice
elif city_flight.lower()=="amsterdam":
        flight_cost=amsterdam_hotelprice
        rental_cost=amsterdam_rentalprice
else:print("destination is not available")
# function to get the total price for the hotel 
def hotel_cost_f(num_nights,flight_cost):
    return num_nights*flight_cost

hotel_price=hotel_cost_f(num_nights,flight_cost)


#function to get the flight price
...
print(" and £",rental_price,"for car rental and also £",plane_price,"for the flight ticket")